...
- **Purpose**: Allow users to customize app appearance, configure SMS messaging, manage message templates, respect customer communication preferences, and export data for backup or analysis
- **Trigger**: Navigate to Settings from sidebar; switch between tabs (General, SMS, Templates, Opt-Outs)
- **Progression**: View settings → General tab: Adjust theme colors → Save theme changes → SMS tab: Configure Twilio credentials → Save SMS config → Templates tab: Create/edit SMS templates with variables → Set template active/inactive → Duplicate templates → Opt-Outs tab: Search customers → View SMS preferences → Manage opt-outs → Toggle message type permissions → Export data to CSV (quotes/jobs/customers)
- **Success criteria**: Theme colors update in real-time; color changes persist; Twilio configuration validates and saves; SMS status indicator shows enabled/disabled; SMS templates support variables like {customer_name}, {amount}, {quote_number}; character count and segment count displayed; templates can be activated/deactivated; customer opt-out list searchable; opt-out status clearly indicated; can manage customer preferences (payment reminders, order updates, marketing); opt-out reasons recorded; compliance warnings displayed; CSV exports download correctly; export includes all relevant data fields

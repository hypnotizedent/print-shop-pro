...
### Artwork Upload with File Size Recognition & Multi-Step Approval Workflow
- **Functionality**: Upload artwork files for each print location with automatic file size detection and display; visual mockup preview with imprint size estimation based on file size; multi-stage approval workflow with internal review, customer review, and final approval stages; assign reviewers to artwork; track approval comments and feedback; maintain approval history with timestamps and reviewer information
- **Purpose**: Track artwork files for production with visibility into file sizes for quality and storage management; provide visual representation of print areas and estimated dimensions; ensure thorough quality control through structured review process before production; facilitate collaboration between internal team, customers, and managers; maintain clear audit trail of all approval decisions and feedback
- **Trigger**: Click upload area in line item decoration section or job detail; hover over preview icon in line item grid; click "Approval Workflow" option in artwork library dropdown menu
- **Progression**: Click upload area → Select image file(s) → File uploads → File size automatically detected and displayed → Thumbnail shows with file info → Hover preview icon shows mockup with all artwork locations → Imprint dimensions estimated from file size → Click "Approval Workflow" → View current review stage (uploaded → internal review → customer review → final approval → approved/rejected) → Assign reviewers (internal team, customer, manager) with optional email notification → Add comments and feedback at any stage → Approve or reject current stage → If approved, advance to next stage → If rejected, mark as rejected with reason → Track all comments and feedback chronologically → View approval history with stages, approvers, dates, and comments → When final approval complete, mark artwork as production-ready
- **Success criteria**: File size displays in human-readable format (KB/MB); supports drag and drop; shows file size on artwork thumbnail; bulk upload supports multiple files; file size stored with artwork metadata; hovering over preview icon shows large mockup with artwork details and estimated imprint sizes (width × height in inches); dimensions calculated based on file size ranges; preview shows all artwork locations with color-coded status; workflow dialog shows visual progress through 5 stages (uploaded, internal review, customer review, final approval, approved); current stage highlighted with distinct color and icon; completed stages show checkmark; rejected artworks show warning icon; can assign multiple reviewers per artwork with roles (internal/customer/manager); reviewer assignments include name, email, and role; optional email notifications sent when reviewers added; approve/reject buttons available at each stage; approval advances to next stage automatically; rejection sets status to 'rejected' and requires rejection reason; all comments timestamped with reviewer name and stage; comments searchable and filterable by stage; approval history shows complete timeline of all actions; production-ready badge appears when final approval complete; workflow state persists with artwork file; can add comments without approving/rejecting
...
### Purchase Order Management & Inventory Receiving
- **Functionality**: Create purchase orders from approved quotes/jobs, consolidate multiple orders into single supplier POs, track receiving inventory with assignment to specific orders
- **Purpose**: Solve the challenge of ordering products for multiple quotes in one purchase order from suppliers (S&S Activewear, SanMar); help receiving team know which garments belong to which customer orders; maintain accurate inventory tracking and cost management
- **Trigger**: Navigate to Settings > Purchase Orders tab; click "New Purchase Order" button
- **Progression**: Create PO → Select supplier (S&S or SanMar) → Select multiple quotes/jobs to include → System consolidates products by style/color → Shows which orders need each product → Review consolidated line items with breakdown by quote/job → Enter order details (date, expected delivery, tracking, shipping, tax) → Save as draft or mark as ordered → When inventory arrives, click "Receive" on PO → Enter quantities received by size → Optionally assign received items to specific quote/job → Mark who received and when → System updates PO status (partially received/received) → Receiving team can view which garments are for which customer orders
- **Success criteria**: Can select multiple quotes/jobs from list showing quote/job number and customer name; consolidation groups identical products (same style, color) across orders with size breakdown; line items show "Associated Orders" section listing all quotes/jobs that need those products; each associated order shows its size breakdown; can add shipping and tax to calculate total PO cost; PO generates unique number (PO-XXXXXXXX); can save as draft and edit later; can mark as ordered with expected delivery date; receiving dialog shows all line items with size input fields; can quickly fill quantities by selecting a specific order (autofills that order's size breakdown); can assign received garments to specific quote/job for receiving team clarity; tracks partial receives over multiple sessions; displays progress bar showing received/ordered quantities; "Receive All Remaining" button autofills outstanding quantities; search works across PO number, product names, supplier, and customer names; status filters (draft, ordered, partially received, received, cancelled) update counts dynamically; PO cards show supplier badge, order date, expected delivery, total cost, and receiving progress; expanding PO card shows full line item details with associated orders; purchase order data persists; receiving history tracked with dates and receiving staff names; received items marked with assignment make it easy for team to know "these 50 shirts are for Quote Q-12345 - ABC Company"
